node {
   ...
   stage('Deploy Spring Boot Application') {
        ...
        sh "docker run --name java-deploy-container --volumes-from maven-build-container -d -p 8080:8080 tomcat:8.0.51-jre8-alpine cp /deploy/application/target/ROOT.war /usr/local/tomcat/webapps/"
